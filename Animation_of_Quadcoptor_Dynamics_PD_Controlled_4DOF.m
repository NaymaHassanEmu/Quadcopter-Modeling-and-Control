...
% 0. Data Pre-processing (2X Real-Time)
FPS = 30; % Frames per second in the video file
SpeedFactor = 2; % Set to 2 for double speed
% The time vector now jumps by (2/30) seconds per frame
t_2x = 0 : (SpeedFactor/FPS) : t(end); 

% Interpolate states to match this accelerated time vector
states_interp = interp1(t, states, t_2x);


% 1. FIRST ANIMATION: Standard View (Real-Time)
v1 = VideoWriter('animation1.mp4', 'MPEG-4');
v1.FrameRate = FPS; 
open(v1);
...
for i = 1:length(t_2x)
    ...
    title(sprintf('Animation: %.2f s', t_2x(i)));
...
v2 = VideoWriter('animation2_detailed.mp4', 'MPEG-4');
v2.FrameRate = FPS;
open(v2);
...
for i = 1:length(t_2x)
    ...
    title(sprintf('3D Quadcopter Animation: %.2f s', t_2x(i)));
...
v3 = VideoWriter('animation3_path.mp4', 'MPEG-4');
v3.FrameRate = FPS;
open(v3);
...
for i = 1:length(t_2x)
    ...
    title(sprintf('3D Path of Quadcopter: %.2f s', t_2x(i)));
    drawnow;
